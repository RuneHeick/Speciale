...
Household 3,10 and 18 are some of the houses that have the smallest \df{other} category. They are therefore selected for further study, since they provide the most information about the house. 
...
In figure \ref{fig:Norm2} is the same signal as in figure \ref{fig:Norm1} filtered by using the purge and merge process. The purge and merge process can be used to create a norm filter, that filters appliance usage that does not fit the norm. As seen in figure \ref{fig:Norm2} does the signal inferred by the filtered \ab{FHMM} algorithm match more closely to the actual signal. 
